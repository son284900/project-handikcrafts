<?php
require_once('../admin/adminheader.php');
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <title>FAQs</title>
    <style>
        table th{
            background-color:lightblue;
        }
    </style>
</head>
<body>
    <?php if(isset($_SESSION['Update'])): ?>    
        <h1 style="text-align:center;border:medium solid green;color:green;padding:10px 0;width:500px;margin:auto;"><?php echo $_SESSION['Update']; ?></h1>
    <?php endif;?>
    <?php if(isset($_SESSION['delete'])): ?>
        <h1 style="text-align:center;border:medium solid green;color:green;padding:10px 0;width:500px;margin:auto;"><?php echo $_SESSION['delete']; ?></h1>
    <?php endif;?>
    <a href="new.php" style="text-align:center;"><h1>Create new FAQs</h1></a> <br><br>
    <table class="table table-striped">
        <tr>
            <th>FAQsID</th>
            <th>Question</th>
            <th>Answer</th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
  	    </tr>

        <?php  
        $faqs_set = find_all_faqs();
        $count = mysqli_num_rows($faqs_set);
        for ($i = 0; $i < $count; $i++):
            $faqs = mysqli_fetch_assoc($faqs_set); 
        ?>
            <tr>
                <td><?php echo $faqs['FAQsID']; ?></td>
                <td><?php echo $faqs['Question']; ?></td>
                <td><?php echo $faqs['Answer']; ?></td>
                <td><a href="<?php echo 'edit.php?id='.$faqs['FAQsID']; ?>">Edit</a></td>
                <td><a href="<?php echo 'delete.php?id='.$faqs['FAQsID']; ?>">Delete</a></td>
            </tr>
        <?php 
        endfor; 
        mysqli_free_result($faqs_set);
        ?>
    </table>
</body>
</html>

<?php
db_disconnect($db);
unset($_SESSION['Update']);
unset($_SESSION['delete']);
?>